...
export const WeatherSummaryInfo = ({ data }: { data: WeatherEntity }) => {
  const {
    formattedDate,
    formattedIconURL,
...
export const WeatherSummary = async ({ lon, lat }: { lat: string; lon: string }) => {
  ...
  return (
    <section>
      <WeatherSearchCard className="p-4 min-h-[230px]" data-testid="search-summary">
        {response?.data && <WeatherSummaryInfo data={response.data} />}
      </WeatherSearchCard>
