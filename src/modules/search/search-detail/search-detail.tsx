...
import { ForecastEntity } from "@/schemas/weather/forecast.schema";
import { GetForecastResponse } from "@/schemas/weather/get-forecast-repsonse.schema";
import { weatherService } from "@/services";
import { groupForecastByDay } from "@/utils";

export const SearchDetailInfo = ({ data }: { data: ForecastEntity[] }) => {
...
export const SearchDetail = async ({ lon, lat }: { lat: string; lon: string }) => {
  ...
  return (
    <>
      <SearchCard className="mt-4 p-4 min-h-1/2" data-testid="search-summary">
        {response && <SearchDetailInfo data={response.data} />}
      </SearchCard>
